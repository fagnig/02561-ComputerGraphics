...
var verts_per_bezier = 64;

var max_objs = max_points + max_circles + max_tris + max_bezier;
...
var bezier_start = triangle_start + verts_per_bezier*max_bezier;
var max_verts = (bezier_start + max_tris * 3) + helperpoints;
...
window.onload = function()
{
  ...
  function color_lerp(c1, c2, lerp)
  {
    var rcol = [
      (c2[0]-c1[0]) * lerp + c1[0],
      (c2[1]-c1[1]) * lerp + c1[1],
  ...
  function add_3pbezier(inparam)
  {
    var step = 1/64;
    var index = bezier_start + verts_per_bezier * n_bezier;
    var col1 = inparam[0].col; var col2 = inparam[1].col;
    var pos1 = inparam[0].pos; var pos2 = inparam[2].pos;
    var pos3 = inparam[1].pos;

    for (var i = 0; i < verts_per_bezier; i++) {
      var t = i * step;

      var pos = [
        Math.pow(1-t,2)*pos1[0] + (0.7071)*(1-t)*2*t*pos2[0] + Math.pow(t,2)*pos3[0],
        Math.pow(1-t,2)*pos1[1] + (0.7071)*(1-t)*2*t*pos2[1] + Math.pow(t,2)*pos3[1],
        curz
      ];
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
      gl.bufferSubData(gl.ARRAY_BUFFER, (index+i)*sizeof['vec3'], flatten(pos));
      gl.bindBuffer(gl.ARRAY_BUFFER, colbuffer);
      gl.bufferSubData(gl.ARRAY_BUFFER, (index+i)*sizeof['vec3'], flatten(color_lerp(col1,col2,i*step)));
    }

    n_bezier = (n_bezier + 1) % max_bezier;
    curz += z_lerp;
  }

  function add_4pbezier(inparam)
  {
    var step = 1/64;
    var index = bezier_start + verts_per_bezier * n_bezier;
    var col1 = inparam[0].col; var col2 = inparam[1].col;
    var pos1 = inparam[0].pos; var pos2 = inparam[2].pos;
    var pos3 = inparam[3].pos; var pos4 = inparam[1].pos;

    for (var i = 0; i < verts_per_bezier; i++) {
      var t = i * step;

      var pos = [
        Math.pow(1-t,3)*pos1[0] + 3*Math.pow(1-t,2)*t*pos2[0] + 3*Math.pow(t,2)*(1-t)*pos3[0] + Math.pow(t,3)*pos4[0],
        Math.pow(1-t,3)*pos1[1] + 3*Math.pow(1-t,2)*t*pos2[1] + 3*Math.pow(t,2)*(1-t)*pos3[1] + Math.pow(t,3)*pos4[1],
        curz
      ];
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
      gl.bufferSubData(gl.ARRAY_BUFFER, (index+i)*sizeof['vec3'], flatten(pos));
  ...
  canv.addEventListener("click", function() {
    ...
    if(drawmode == "Quadr. Bezier Curve")
    {
      tempshapepoints.push({pos: t, col: pointcol});
      add_helper(t, pointcol);
      if(tempshapepoints.length == 3)
      {
        add_3pbezier(tempshapepoints);
        tempshapepoints = [];
        n_helperpoints = 0;
      }
    }
    if(drawmode == "Cubic Bezier Curve")
    {
      tempshapepoints.push({pos: t, col: pointcol});
      add_helper(t, pointcol);
      if(tempshapepoints.length == 4)
      {
        add_4pbezier(tempshapepoints);
        tempshapepoints = [];
        n_helperpoints = 0;
      }
    }
...
function render()
{
  ...
  if(n_bezier > 0){
    for (var i = 0; i < n_bezier; i++) {
      gl.drawArrays(gl.LINE_STRIP, bezier_start + verts_per_bezier*i, verts_per_bezier);
    }
  }
